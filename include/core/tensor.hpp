...
public:
  ...
  std::array<Tensor*, TENSOR_MAX_SRC> src = {nullptr}; // source tensors
  ...
  Tensor(const std::string& name, int layer_id, DataType dtype, TensorType type, OperationType op_type):
    // ReSharper restore CppParameterMayBeConst
    op_type(op_type), dtype(dtype), type(type), layer_id(layer_id), name(name) {}
  ...
  [[nodiscard]] size_t nbytesAt(int64_t resolve) const {
    size_t total = 1;
    for (int i = 0; i < TENSOR_MAX_DIMS && dims[i] != 0; ++i) {
